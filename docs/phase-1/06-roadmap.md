...
| **3** Big number | `NE.Big` (FFI, OmegaNum inline, jalur cepat, mutasi), notasi, pack/unpack, wrap `number_format`/`math.*`/high score, patch L6–L8, rehydrate (wrap `STR_UNPACK`/`start_run`, menggantikan L4); tes CLI | Cheat: set skor `1e500`, target `ee10`, `10^^5`; lihat tampilan, save & load, lanjutkan | Semua tes CLI lulus; save/load bolak-balik tanpa kehilangan nilai; tidak ada `inf`/`nan`; tes runtime `__eq` dicatat |
...
| **5** Papan | `G.play` → papan 5×3, slot di `ability`, wrap `align_cards`/`emplace`, penempatan drag & klik (kartu staged tetap di tangan), main cepat otomatis, residu + batas residu & L13a–c/L14, discard kartu papan, efek baris, navigasi kontroler, `NE.Board.resize`, tata letak (ruang game tetap; 4:3 = letterbox) | Main beberapa tangan; residu tetap; save di tengah ronde lalu load; coba dengan kontroler | Tidak ada soft-lock; posisi pulih setelah load; layout muat di 1280×720 |
| **6** Formasi | Tabel pola, evaluator + cache, 17 formasi (`SMODS.PokerHand`), rantai sekunder (fase Rantai), pratinjau yang ikut posisi, 17 planet + sprite, diagram mini di Run Info, save lama, L12a–b | Bentuk tiap formasi; lihat nama rantai; pakai planet; buka Run Info | Tes CLI evaluator lulus; semua formasi terdeteksi benar di game |
